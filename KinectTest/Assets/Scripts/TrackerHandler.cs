...
public class TrackerHandler : MonoBehaviour
{
    ...
    {
        float dist = Vector3.Distance(transform.GetChild(0).GetChild(0).position, new Vector3(-0.2f, -0.3f, -0.1f));
        if(dist<1.7f)
        {
            if(VideoImg.activeSelf)
            {
        ...
        RealBody = closestBody;
        PersonCount = closestBody;
        if (closestBody == -1)
        {
            int IndexBody = findIndexFromId(trackerFrameData, IndexCount);

            if (IndexBody == -1)
            {
                PersonCount = 0;
                IndexBody = findIndexFromId(trackerFrameData, (int)trackerFrameData.Bodies[PersonCount].Id);
                IndexCount = (int)trackerFrameData.Bodies[PersonCount].Id;
            }
            RealBody = IndexBody;
        }

        var pelvisPosition = trackerFrameData.Bodies[RealBody].JointPositions3D[(int)JointId.Pelvis];
        Vector3 pelvisPos = new Vector3((float)pelvisPosition.X, -0.3f, (float)pelvisPosition.Z);
        float CheckdistX = pelvisPos.x + 0.2f;
        float CheckdistZ = pelvisPos.z - 1.8f;
        if ((CheckdistX<=0.2f || CheckdistX>=0.9f) || Mathf.Abs(CheckdistZ)>=0.3f)
        {
        ...
        {
            Cube.SetActive(true);
            Body skeleton = trackerFrameData.Bodies[RealBody];
        ...
        {
            print("두명");
            int IndexBody2 = findSecondClosestTrackedBody(trackerFrameData);
            try
            {
                var pelvisPosition2 = trackerFrameData.Bodies[RealBody].JointPositions3D[(int)JointId.Pelvis];
                Vector3 pelvisPos2 = new Vector3((float)pelvisPosition2.X, -0.3f, (float)pelvisPosition2.Z);
                float CheckdistX2 = pelvisPos2.x + 0.2f;
                float CheckdistZ2= pelvisPos2.z - 1.8f;
                if ((CheckdistX2 <= 0.2f && CheckdistX2 >= 0.9f) || Mathf.Abs(CheckdistZ2) >= 0.3f)
                {
                    Second.SetActive(false);
                }
                else
                {
                    Body skeleton2 = trackerFrameData.Bodies[IndexBody2];
                    renderSkeletonSecond(skeleton2, 0);
    ...
    {
        int retIndex = -1;
        print("Count = " + frameData.NumOfBodies+"명");
    ...
    private int findSecondClosestTrackedBody(BackgroundData trackerFrameData)
    {
        int closestBody = -1;
        int secondClosestBody = -1;
        const float MAX_DISTANCE = 5000.0f;
        float minDistanceFromKinect = MAX_DISTANCE;
        for (int i = 0; i < (int)trackerFrameData.NumOfBodies; i++)
        {
        ...
        minDistanceFromKinect = MAX_DISTANCE;
        for (int i = 0; i < (int)trackerFrameData.NumOfBodies; i++)
        {
            var pelvisPosition = trackerFrameData.Bodies[i].JointPositions3D[(int)JointId.Pelvis];
            Vector3 pelvisPos = new Vector3((float)pelvisPosition.X, (float)pelvisPosition.Y, (float)pelvisPosition.Z);
    ...
    public void renderSkeletonSecond(Body skeleton, int skeletonNumber)
    {
        for (int jointNum = 0; jointNum < (int)JointId.Count; jointNum++)
        {
            Vector3 jointPos = new Vector3(skeleton.JointPositions3D[jointNum].X, -skeleton.JointPositions3D[jointNum].Y, skeleton.JointPositions3D[jointNum].Z);
            Vector3 offsetPosition = Second.transform.rotation * jointPos;
            Vector3 positionInTrackerRootSpace = Second.transform.position + offsetPosition;
            Quaternion jointRot = Y_180_FLIP * new Quaternion(skeleton.JointRotations[jointNum].X, skeleton.JointRotations[jointNum].Y,
                skeleton.JointRotations[jointNum].Z, skeleton.JointRotations[jointNum].W) * Quaternion.Inverse(basisJointMap[(JointId)jointNum]);
            absoluteJointRotations[jointNum] = jointRot;
            // these are absolute body space because each joint has the body root for a parent in the scene graph
            Second.transform.GetChild(skeletonNumber).GetChild(jointNum).localPosition = jointPos;
            Second.transform.GetChild(skeletonNumber).GetChild(jointNum).localRotation = jointRot;
            ...
            if (parentJointMap[(JointId)jointNum] != JointId.Head && parentJointMap[(JointId)jointNum] != JointId.Count)
            {
                Vector3 parentTrackerSpacePosition = new Vector3(skeleton.JointPositions3D[(int)parentJointMap[(JointId)jointNum]].X,
                    -skeleton.JointPositions3D[(int)parentJointMap[(JointId)jointNum]].Y, skeleton.JointPositions3D[(int)parentJointMap[(JointId)jointNum]].Z);
                Vector3 boneDirectionTrackerSpace = jointPos - parentTrackerSpacePosition;
                Vector3 boneDirectionWorldSpace = Second.transform.rotation * boneDirectionTrackerSpace;
                Vector3 boneDirectionLocalSpace = Quaternion.Inverse(Second.transform.GetChild(skeletonNumber).GetChild(jointNum).rotation) * Vector3.Normalize(boneDirectionWorldSpace);
                Second.transform.GetChild(skeletonNumber).GetChild(jointNum).GetChild(boneChildNum).localScale = new Vector3(1, 20.0f * 0.5f * boneDirectionWorldSpace.magnitude, 1);
                Second.transform.GetChild(skeletonNumber).GetChild(jointNum).GetChild(boneChildNum).localRotation = Quaternion.FromToRotation(Vector3.up, boneDirectionLocalSpace);
                Second.transform.GetChild(skeletonNumber).GetChild(jointNum).GetChild(boneChildNum).position = Second.transform.GetChild(skeletonNumber).GetChild(jointNum).position - 0.5f * boneDirectionWorldSpace;
            }
            else
            {
                //Second.transform.GetChild(skeletonNumber).GetChild(jointNum).GetChild(boneChildNum).gameObject.SetActive(false);
            }
        }
    }
